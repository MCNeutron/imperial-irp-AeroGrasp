...
def init_model():
    ...
    #config = config.get_config("pi0_fast_libero_low_mem_finetune") # ADDED
    ...
    checkpoint_dir = "/rds/general/user/ll1225/home/imperial_irp/extended_evo1/weights/indooruav_modelscope/29999" # ADDED
...
def infer(policy, inputs):
    # DEBUGGING
    ####################
    outputs = policy.infer(inputs)["actions"]
    with open(DEBUG_FILE_DIR, "a") as f:
        f.write("\n" + "=" * 80 + "\n") # Separator

        # Log Evo1-format inputs
        f.write(">> OUTPUTS <<\n")
        f.write(f"outputs: {np.array(outputs).tolist()}\n")
...
SHARED_FOLDER = "/rds/general/user/ll1225/home/imperial_irp/extended_evo1/shared_folder" ### ADDED for testing AGVLA
...
print("SHARED resolved =", os.path.abspath(SHARED_FOLDER), flush=True)
###
