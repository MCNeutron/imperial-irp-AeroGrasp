...
def get_glb_path(group, scene):
    ...
    if dataset_type == 'mp3d':
        ...
        return f"{SCENE_BASE_DIR}/replica/{scene}/habitat/mesh_preseg_semantic.ply" ### ADDED
    ### ADDED
    #elif dataset_type == "habitat-test-scenes":
    #    return f"{SCENE_BASE_DIR}/habitat-test-scenes/{scene}.glb"
    ###
    else:
...
def is_success(current_coords, target_coords, pos_threshold=0.1, angle_threshold=np.pi/6):
    """
    检查是否成功到达目标位置
